...
namespace Core.Processes
{
    public class ProcessStartHelper
    {
        ...
        /// </summary>
        public static void ExecuteProcessUnElevated(string filePathProcess, string args, string currentDirectory = "")
        {
            IShellWindows shellWindows = (IShellWindows)new CShellWindows();
            IServiceProvider serviceProvider = GetDesktopWindow(shellWindows);
            IShellBrowser shellBrowser = GetShellBrowser(serviceProvider);
            IShellDispatch2 shellDispatch = GetShellDispatch(shellBrowser);

            // Use the dispatch (which is unelevated) to launch the process for us
            shellDispatch.ShellExecute(filePathProcess, args, currentDirectory, string.Empty, SW_SHOWNORMAL);
        }
        private static IServiceProvider GetDesktopWindow(IShellWindows shellWindows) {
            object loc = CSIDL_Desktop;
            object unused = new object();
            int hwnd;
            return (IServiceProvider)shellWindows.FindWindowSW(ref loc, ref unused, SWC_DESKTOP, out hwnd, SWFO_NEEDDISPATCH);
        }
        private static IShellBrowser GetShellBrowser(IServiceProvider serviceProvider)
        {
            Guid serviceGuid = SID_STopLevelBrowser;
            Guid interfaceGuid = typeof(IShellBrowser).GUID;
            return (IShellBrowser)serviceProvider.QueryService(ref serviceGuid, ref interfaceGuid);
        }
        private static IShellDispatch2 GetShellDispatch(IShellBrowser shellBrowser) {

            Guid dispatch = typeof(IDispatch).GUID;
            IShellFolderViewDual folderView = (IShellFolderViewDual)shellBrowser.QueryActiveShellView().GetItemObject(SVGIO_BACKGROUND, ref dispatch);
